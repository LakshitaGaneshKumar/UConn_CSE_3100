...
node *delete_node(node *head, int v) {
    // If the value is in the head node, free the head and return the node after the head
    if (head->v == v) {
    ...
    // Iterate through linked list to find the node
    node *prev = head;
...
node *reverse_list(node *head) {
    // Initialize current, prev, and next node pointers
    node *current = head;
    node *prev = NULL;
    node *next = NULL;
